...
//window.addEventListener('load', function () {
 $(document).ready(function() {
    ...
    $("#btnStateClaim").click(function() { 
    var stateAddress = $('#stateaddress').val();
    $("#divClaimDetailsOfState").show();      
    care.ClaimCreated({memberAddress: web3.eth.accounts[0]}, {
        fromBlock: 0,
        toBlock: 'latest'
        ...
        if (error)
            ...
            //filter the data 
             stateEventResult=  $(eventResult)
                  .filter(function (index,claim){
                      return (claim.args.stateAddress).toLowerCase() === stateAddress.toLowerCase();
                      
                  }); 
            //end of filter
            $("#tblState").DataTable({
                data: stateEventResult,
                "bDestroy": true,
                "columns": [{
                        "data": "args.claimID"
                    },
